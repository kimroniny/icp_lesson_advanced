import { wallet_multisig, idlFactory, canisterId as main_canister_id } from "../../declarations/wallet_multisig";
...
function App() {
  ...
  const getData = async () => {
    try {
      let team = await wallet_multisig.get_owner_list();
      setTeam(team);
  
      let proposals = await wallet_multisig.get_proposals();
      proposals.sort((x,y) => {
        let m = parseInt(x.id);
        let n = parseInt(y.id);
        if (m < n) return 1;
      ...
      let r = await wallet_multisig.get_model();
      setM(r[0].toString());
      setN(r[1].toString());
  
      let canisters = await wallet_multisig.get_owned_canisters_list();
      let canistersM = new Array(canisters.length);
      let canistersStatus = new Array(canisters.length);
      for(var i = 0; i < canisters.length; i++) {
        canistersM[i] = await wallet_multisig.get_permission(canisters[i]);
        canistersStatus[i] = await wallet_multisig.get_status(canisters[i]);
      }
    ...
    <div style={{ "fontSize": "20px" }}>
      ...
      <div>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title><h4>A problem occurred!!!</h4></Modal.Title>
          </Modal.Header>
          <Modal.Body><h4>pls log in or check if you're one of administrators</h4></Modal.Body>
          <Modal.Footer>
      ...
      <div>
        ...
        >
          ...
          <Modal.Body>
            <div>
              <button onClick={() => openFileSelector()}>choose wasm code</button>
              <br />
      ...
      <div style={{ "fontSize": "20px" }}>
        <p align="center"><b>
        {logined? "User Principal：": "No Logged In!"}
        {principal} {''}
        {
          logined
          ? <Button variant="success" onClick={handleLogoutClick}>log out</Button>
          : <Button variant="success" onClick={handLoginClick}>IIs log in </Button>
        }
        </b></p>
      </div>

      {/* <div style={{ "backgroundColor": "#e0b0ab", "fontSize": "20px" }}>
        <p><b>wallet multi_sig</b></p>
      </div> */}

      <div style={{ "fontSize": "20px" }}>
          <p><b> &gt;&gt;&gt; Start from Here! </b></p>
        </div>

      <div style={{"margin": "20px auto"}}>
        <Button disabled={processing} variant="primary" onClick={handleCreateCanister}>
          { createCanister ?
            <Spinner
              as="span"
              animation="grow"
              ...
              canisters.map((t,index) => {
                return (
                  <tr key={t.toText()}>
                    ...
                    <td>
                      <div>
                        <Button disabled={processing} variant="primary" onClick={handleCodeAction.bind(this, t.toText(), "installCode")}>
                        ...
                        <Button disabled={processing} variant="primary" onClick={handleCanisterAction.bind(this, t.toText(), "startCanister", null)}>
                        ...
                        <Button disabled={processing} variant="primary" onClick={handleCanisterAction.bind(this, t.toText(), "stopCanister", null)}>
                        ...
                        <Button disabled={processing} variant="primary" onClick={handleCanisterAction.bind(this, t.toText(), "deleteCanister", null)}>
      ...
      <div style={{ "fontSize": "20px" }}>
        ...
        <table className="table table-striped">
            ...
            <tr>
                ...
                <td width="250">principal</td>
                <td width="300">issuer</td>
                <td width="300">approver</td>
                <td width="300">refuser</td>
                <td width="100">wasm hash</td>
                ...
                proposals.map(data => {
                    return (
                        <tr key={data.id.toString()}>
                            ...
                            <td width="300">
                            {
                              principal === data.proposer.toText() ? "myself": principal_to_short(data.proposer)
                            }
                            </td>
                            <td width="300">
                                {
                                    data.approvers.map(a => {
                                        return (
                                            <li key={principal_to_short(a)}>
                                              {
                                                principal === a.toText() ? "myself": principal_to_short(a)
                                              }
                                            </li>
                                        )
                                    })
